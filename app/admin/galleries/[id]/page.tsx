  ...
  return (
    <main className="adminPage">
      ...
      <section className="adminPanel">
        ...
        {selectedImages.length ? <div className="adminList">{selectedImages.map((image, index) => <div className="adminListItem" key={image.id}><div><strong>{String(index + 1).padStart(2, "0")} · {image.filename}</strong><span>Image ID · {image.id}</span></div><span>Selected</span></div>)}</div> : <div className="adminEmptyState"><h3>No selections yet</h3><p>The client&apos;s chosen photographs will appear here as they review the gallery.</p></div>}
      </section>
      <section className="adminPanel">
